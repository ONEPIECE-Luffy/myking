...
public class pageQueryOrder extends AbstractService {
	...
	public String execute(String reqXml) throws Exception {
		...
		logger.error("分页查询接口【support.pageQueryOrder】-->【"+hisInterface+"】入参："+inputString);
		...
		String resCode=UtilXml.getValueByAllXml(resultXml, "resultCode");//处理结果代码：0-成功
		logger.error("分页查询接口【support.pageQueryOrder】-->【"+hisInterface+"】出参："+resultXml);
		if("-1".equals(resCode)){
	...
	private String getInputParamString(String reqXml){
		String tradeDate=UtilXml.getValueByAllXml(reqXml, "tradeDate");//交易日期
		String hisOrdNum=UtilXml.getValueByAllXml(reqXml, "orderId");//移动支付订单号
