...
public class getRegisterInfo extends AbstractService {
	...
	public String execute(String reqXml) throws Exception {
		...
		//String resultXml = "<response><resultCode>0</resultCode><resultMessage>调用接口正常。</resultMessage><result><item><hisOrdNum>5047829558</hisOrdNum><hisRefOrdNum></hisRefOrdNum><deptCode>8124</deptCode><deptName>心内科门诊</deptName><doctorCode></doctorCode><doctorName></doctorName><bookDate>2018-02-06</bookDate><timeFlag>2</timeFlag><beginTime>16:30</beginTime><endTime>17:00</endTime><regType>1</regType><orderMode>2</orderMode><payMode>1</payMode><status>8</status><realRegFee>0</realRegFee><realTreatFee>10000</realTreatFee><desc></desc><receiptNum>R00000082635</receiptNum><serialNum></serialNum><visitLocation>二楼</visitLocation><takeTime>2018-02-06 16:00:00</takeTime><barCode>AK2331553</barCode><visitDesc>请在预约时间段开始时间提前半个小时来医院二楼分诊台报到候诊。</visitDesc></item><item><hisOrdNum>49653296";
		String resCode=UtilXml.getValueByAllXml(resultXml, "resultCode");
			if("0".equals(resCode)){
				...
				//getAppointmetOrder(reqXml,otherRegList);//获取患者就诊日期是今天以后的记录，然后是否支付直接设置为 0,  平台现在默认未来的号源不能支付，所以这个方法其实可以删除，删除了务必测试，否则会导致严重问题，影响某些患者医保挂号后诊间支付无法医保结算
	...
	private GetRegisterInfoResultV3 convertResult(String reqXml, String resultXml) throws Exception {
		...
		GetRegisterInfoResultV3 resultV3 = convertGetRegRecordsToV3Object(reqXml,resultXml);
		 ...
		 //getAppointmetOrder(reqXml,list); //获取患者就诊日期是今天以后的预约记录，然后是否支付直接设置为 0,  平台现在默认未来的号源不能支付，所以这个方法其实可以删除，删除了务必测试，否则会导致严重问题，影响某些患者医保挂号后诊间支付无法医保结算
...
//			String  mzlsh = getMzlsh(ele,isOther);
			...
			logger.info(bookingNo+" 号源开始时间是 "+ele.elementText("beginTime")+ "  "+startTime);
			logger.info("当前时间是 "+sdf.format(new Date())+ "  "+now);
			...
			if("0".equals(status)){//   	0 未付费        判断用户是否是医保预约取号，如果是，要进行限制必须就诊当日才可以进行支付
				if(today.equals(regData)){ 
					...
					/*if((startTime-now)>1000*60*30){  //当日就诊当前，就诊前小于半小时不允许线上支付 
						logger.info(bookingNo+" 号源开始时间 大于 当前时间 30分钟，可以允许患者支付");
						...
						logger.info(bookingNo+" 号源开始时间  小于 当前时间 30分钟，不允许患者支付");
				...
				resultItem.setIsSelectSvObject("0");
				resultItem.setIsPayment("0"); // isPayment  是否允许支付【0-不允许；1-允许
				resultItem.setPayStatus("2"); // 0-未支付 1-已支付  2-已退费
			}
			 

			if(StringUtil.isEmpty(orderId)){ //如果是挂号订单详情需要返回下面字段
			  //加入订单类类型字段
				//如果是非微信渠道，先调用锁号接口
				resultItem.setIsCancelabe("0");
		...
		for(Element ele:list){
			...
			if("1".equals(status)){ //1：未支付  2：已支付 3：已取消4：缴费超时5：已取号6：已就诊7：已过期8：已退费
				if(today.equals(regData)){ 
					...
					resultItem.setIsCancelabe("1");
					//resultItem.setIsSelectSvObject("0"); //暂时不允许选择医保对象
				/*	if((now-startTime)>1000*60*30){  //当日就诊当前，就诊前小于半小时不允许线上支付
						//----就诊当天，就诊时间大于就诊前半小时，普通患者和就诊患者都可以支付--------//
						logger.info(bookingNo+" 号源开始时间  大于 当前时间 30分钟，允许患者支付");
						resultItem.setIsPayment("1"); // isPayment  是否允许支付【0-不允许；1-允许
						resultItem.setPayStatus("0"); //0-未支付 1-已支付  2-已退费
						//resultItem.setIsSelectSvObject("1");//允许选择优惠对象
						resultItem.setIsSelectSvObject("0"); //暂时不允许选择医保对象
			...
			if(StringUtil.isEmpty(orderId)){ //如果是挂号订单详情需要返回下面字段,非金蝶渠道MongoDB差找不到orderId
				//resultItem.setAutoLockBeforePay("1"); //modify by CXM
				resultItem.setOrderType("99");
				resultItem.setOrderTypeName("非微信渠道");
				resultItem.setIsCancelabe("0"); //第三方挂号不允许取消
			}else{
				resultItem.setOrderType("10");
				resultItem.setOrderTypeName("微信渠道");
			}
	...
	/**
	 * 通过His的预约号获取订单号,his接口不返回orderId
	 * @author YJB
	 * @date 2017年10月23日 上午11:42:10
	 * @param elementText
	 * @return
	 */
	private String getOrderIdByBookingNo(String bookingNo) {
		...
		if(info!=null){
			logger.info("his流水号为 "+bookingNo+" 的挂号订单，在MongoDB的orderId为 "+info.getOrderId());
			return info.getOrderId();
		}else{
			logger.warn("his流水号为 "+bookingNo+" 的挂号订单，在MongoDB没有存储信息，此号应为【其他渠道号源】");
			return "";
		}
...
public static void main(String[] args) throws Exception {
	...
	String req="<req><healthCardNo>AK2331553</healthCardNo><patientId>0001614642</patientId><patientName>吴文杰</patientName><idCardNo>62050319900215201X</idCardNo></req>";
	String reString = "<response><resultCode>0</resultCode><resultMessage>调用接口正常。</resultMessage><result><item><patCardType>1</patCardType><patCardNo></patCardNo><patName>吴文杰</patName><deptCode>9940</deptCode><deptName>中医科门诊</deptName><doctorCode>9940</doctorCode><doctorName>中医科门诊</doctorName><scheduleDate>2018-02-08</scheduleDate><timeFlag>2</timeFlag><beginTime>16:00</beginTime><endTime>16:30</endTime><workId>6510602</workId><regFee>0</regFee><treatFee>10000</treatFee><SStreatFee>8600</SStreatFee><payFlag>2</payFlag><receiptNum>R00000083544</receiptNum><serialNum></serialNum><visitLocation>一楼</visitLocation><payMode></payMode><regType></regType><orderTime>2018-02-08 15:32:46</orderTime><hisOrderNum>4965329652</hisOrderNum><mzFeeId>6584622</mzFeeId></item><item><patCardType>1</patCardType><patCardNo></patCardNo><patName>吴文杰</patName><deptCode>8124</deptCode><deptName>心内科门诊</deptName><doctorCode>8124</doctorCode><doctorName>心内科门诊</doctorName><scheduleDate>2018-02-08</scheduleDate><timeFlag>2</timeFlag><beginTime>16:00</beginTime><endTime>16:30</endTime><workId>6517192</workId><regFee>0</regFee><treatFee>10000</treatFee><SStreatFee>8600</SStreatFee><payFlag>2</payFlag><receiptNum>R00000083546</receiptNum><serialNum></serialNum><visitLocation>二楼</visitLocation><payMode></payMode><regType></regType><orderTime>2018-02-08 15:34:51</orderTime><hisOrderNum>5047836765</hisOrderNum><mzFeeId>6584648</mzFeeId></item></result></response>";
