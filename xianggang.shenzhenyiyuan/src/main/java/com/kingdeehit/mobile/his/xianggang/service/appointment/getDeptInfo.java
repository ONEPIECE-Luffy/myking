...
public class getDeptInfo extends AbstractService {
	...
	public String execute(String reqXml) throws Exception {		
		...
		logger.error("预约挂号科室查询接口【appointment.getDeptInfo】-->【"+hisInterface+"】入参：user="+ParamConstants.USER+";password="+ParamConstants.PASSWORD+";parameter="+inputString);
		HttpRequestService xmlRequest = HttpRequestService.getInstance();					
		String resultXml= xmlRequest.request(convertInputString);	
		resultXml=CommonUtils.convertABSOutputParam(resultXml);		
		logger.error("预约挂号科室查询接口【appointment.getDeptInfo】-->【"+hisInterface+"】出参："+resultXml);
